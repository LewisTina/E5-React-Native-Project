...
export const auth = {
  ...
  async getTokens(): Promise<AuthTokens | null> {
    ...
    console.log("[AUTH] Getting tokens :", {
      ...
      expiresAt: expiresAt
        ? new Date(parseInt(expiresAt, 10)).toISOString()
        : "none",
    });
    ...
    if (
      accessToken === "SIMULATOR_MOCK_TOKEN" ||
  ...
  async register(
    ...
    try {
      ...
      if (error instanceof TypeError && error.message.includes("fetch")) {
        console.error(
          "❌ [AUTH] Network error - Backend might be down or URL incorrect",
        );
        throw new Error(
          `Cannot connect to server at ${url}. Make sure the backend is running.`,
        );
      }
      if (error instanceof Error) {
        throw error;
